...
class NSQDConnection extends EventEmitter {
  ...
  registerStreamListeners(conn) {
    ...
    return conn.on('close', err => {
      return this.statemachine.raise('close');
    });
  }

  startTLS(callback) {
    for (let event of ['data', 'error', 'close']) { this.conn.removeAllListeners(event); }
  ...
  reconsumeFrameBuffer() {
    if (this.frameBuffer.buffer && this.frameBuffer.buffer.length) {
      let data = this.frameBuffer.buffer;
  ...
  receiveRawData(data) {
    if (!this.inflater) {
      ...
      return this.inflater.write(data, () => {
        let uncompressedData = this.inflater.read();
  ...
  receiveData(data) {
    ...
    return (() => {
      ...
      while (frame = this.frameBuffer.nextFrame()) {
        let item;
        let [frameId, payload] = Array.from(frame);
        switch (frameId) {
  ...
  identify() {
    ...
    for (let key of Array.from(removableKeys)) { if (identify[key] === null) { delete identify[key]; } }
    return identify;
  }
  ...
  createMessage(msgPayload) {
    let msgComponents = wire.unpackMessage(msgPayload);
    let msg = new Message(...msgComponents, this.config.requeueDelay, this.msgTimeout,
      this.maxMsgTimeout);
  ...
  write(data) {
    if (this.deflater) {
      return this.deflater.write(data, () => {
        return this.conn.write(this.deflater.read());
      }
      );
    } else {
    ...
    this.prototype.states = {
      ...
      IDENTIFY: {
        Enter() {
          ...
          let identify = this.conn.identify();
      ...
      IDENTIFY_COMPRESSION_CHECK: {
        Enter() {
          let {deflate, snappy} = this.identifyResponse;
      ...
      READY_SEND: {
        ...
        produceMessages(data) {
          let [topic, msgs, callback] = Array.from(data);
        ...
        response(data) {
          switch (data.toString()) {
            case 'OK':
              let cb = this.conn.messageCallbacks.shift();
      ...
      ERROR: {
        Enter(err) {
          ...
          let errorCode = __guard__(err.split(/\s+/), x => x[1]);
      ...
      CLOSED: {
        Enter() {
          ...
          let err = new Error('nsqd connection closed');
          for (let cb of Array.from(this.conn.messageCallbacks)) {
            if (typeof cb === 'function') {
              cb(err);
            }
          }
