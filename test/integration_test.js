...
const startNSQD = function (dataPath, additionalOptions, callback) {
  ...
  const process = child_process.spawn('nsqd', options.concat(additionalOptions),
...
const topicOp = function (op, topic, callback) {
  const options = {
    method: 'POST',
...
const publish = function (topic, message, done) {
  const options = {
  ...
  return describe('end to end', () => {
    ...
    it('should send and receive a Buffer', (done) => {
      ...
      return reader.on('message', (readMsg) => {
        for (let i = 0; i < readMsg.body.length; i++) { const readByte = readMsg.body[i]; readByte.should.equal(message[i]) }
        readMsg.finish()
        return done()
      })
    })
...
describe('failures', () => {
  ...
  before(done => temp.mkdir('/nsq', (err, dirPath) => startNSQD(dirPath, {}, (err, process) => {
    nsqdProcess = process
    return done(err)
  })))
