...
let matchCommand = function(commandFn, args, expected) {
  let commandOut = commandFn.apply(null, args);
...
describe("nsq wire", function() {
  ...
  return it('should unpack a received message', function() {
    ...
    let msgParts = wire.unpackMessage(new Buffer(msgPayload.join(''), 'hex'));

    let [id, timestamp, attempts, body] = Array.from(msgParts);
