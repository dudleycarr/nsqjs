...
class Reader extends EventEmitter {
  ...
  connect() {
    let delayedStart;
    let interval = this.config.lookupdPollInterval * 1000;
    let delay = Math.random() * this.config.lookupdPollJitter * interval;
    ...
    if (this.config.nsqdTCPAddresses.length) {
      ...
      delayedStart = () => {
        return this.connectIntervalId = setInterval(directConnect.bind(this), interval);
      };
    ...
    // Connect to nsqds discovered via lookupd
      ...
      delayedStart = () => {
        return this.connectIntervalId = setInterval(this.queryLookupd.bind(this), interval);
      };
  ...
  queryLookupd() {
    ...
    let endpoint = this.roundrobinLookupd.next();
    return lookup(endpoint, this.topic, (err, nodes) => {
      return (() => {
        let result = [];
        for (let n of Array.from(nodes)) {           let item;
  ...
  connectToNSQD(host, port) {
    ...
    let conn = new NSQDConnection(host, port, this.topic, this.channel, this.config);
  ...
  registerConnectionListeners(conn) {
    ...
    conn.on(NSQDConnection.CLOSED, () => {
      ...
      let index = this.connectionIds.indexOf(conn.id());
      if (index === -1) { return; }
  ...
  handleMessage(message) {
    ...
    return process.nextTick(() => {
      let autoFinishMessage = 0 < this.config.maxAttempts && this.config.maxAttempts <= message.attempts;
      let numDiscardListeners = this.listeners(Reader.DISCARD).length;
