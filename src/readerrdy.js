...
class ConnectionRdy extends EventEmitter {
  ...
  constructor(conn) {
    super(...arguments);
    this.conn = conn;
    let readerId = `${this.conn.topic}/${this.conn.channel}`;
    let connId = `${this.conn.id().replace(':', '/')}`;
    ...
    this.conn.on(NSQDConnection.ERROR, err => {
      return this.log(err);
    }
    );
    ...
    this.conn.on(NSQDConnection.FINISHED, () => {
      return this.inFlight -= 1;
    }
    );
    this.conn.on(NSQDConnection.REQUEUED, () => {
      return this.inFlight -= 1;
    }
    );
    this.conn.on(NSQDConnection.READY, () => {
      return this.start();
    }
    );
  }
  ...
  isStarved() {
    if (!(this.inFlight <= this.maxConnRdy)) { throw new Error('isStarved check is failing'); }
...
class ConnectionRdyState extends NodeState {
  static initClass() {
    ...
    this.prototype.states = {
      INIT: {
        ...
        bump() {
          if (this.connRdy.maxConnRdy > 0) { return this.goto('MAX'); }
        },
        backoff() {}, // No-op
        adjustMax() {}
      }, // No-op
      ...
      MAX: {
        ...
        bump() {
          ...
          let version = this.connRdy.conn != null ? this.connRdy.conn.nsqdVersion : undefined;
  ...
  log(message) {
    ...
    if (message) { return this.connRdy.debug(message); }
  }
}
...
class ReaderRdy extends NodeState {
  static initClass() {
    ...
    */
    this.prototype.states = {
      ZERO: {
        Enter() {
          if (this.backoffId) { return clearTimeout(this.backoffId); }
        },
        backoff() {},        // No-op
        success() {},        // No-op
        try() {},            // No-op
  ...
  */
  ...
  isStarved() {
    ...
    return !_.isEmpty(((() => {
      let result = [];
      for (let c of Array.from(this.connections)) {         let item;
        if (c.isStarved()) { item = c; }
        result.push(item);
      }
      return result;
    })()));
  ...
  addConnection(conn) {
    let connectionRdy = this.createConnectionRdy(conn);
    ...
    conn.on(NSQDConnection.FINISHED, () => {
      return this.raise('success', connectionRdy);
    }
    );
  ...
  backoff() {
    for (let conn of Array.from(this.connections)) { conn.backoff(); }
    ...
    let delay = new Number(this.backoffTimer.getInterval().valueOf()) * 1000;
    this.backoffId = setTimeout(onTimeout, delay);
    return this.log(`Backoff for ${delay}`);
  }

  inFlight() {
    let add = (previous, conn) => previous + conn.inFlight;
  ...
  */
  balance() {
    ...
    */
    ...
    let max = (() => { switch (this.current_state_name) {
      case 'TRY_ONE': return 1;
      case 'PAUSE': return 0;
      default: return this.maxInFlight;
    } })();

    let perConnectionMax = Math.floor(max / this.connections.length);
    ...
    if (perConnectionMax === 0) {
      ...
      return (() => {
        let result = [];
        for (let i = 0, end = this.connections.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
